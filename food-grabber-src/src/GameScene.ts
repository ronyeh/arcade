...
export class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.targetFoodText = this.add
            .text(this.cameras.main.width / 2, 50, "", { fontSize: "32px", color: "#fff", backgroundColor: "#333" })
            .setOrigin(0.5)
            .setPadding(10);
        ...
        this.input.on(
            "pointerdown",
            (pointer: Phaser.Input.Pointer, gameObjectsClicked: Phaser.GameObjects.GameObject[]) => {
                if (this.isGameOver) {
                    this.restartGame();
                } else {
    ...
    update() {
        ...
        this.foodItemGroup.getChildren().forEach((item) => {
            ...
            if (foodItem.y > this.cameras.main.height + foodItem.height) {
                if (foodItem.active) {
                    // No penalty for missing any fruit, just destroy it
                    foodItem.destroy();
                }
            }
        });
    ...
    private selectNewTargetFood() {
        ...
        this.targetFoodText.setText(
            `Click ${this.currentTargetType.charAt(0).toUpperCase() + this.currentTargetType.slice(1)}!`,
        );
    ...
    private triggerGameOver(clickedItem?: FoodItem) {
        console.log(
            `Incorrect! Clicked: ${clickedItem?.type || "nothing"}, Expected: ${this.currentTargetType}. Game Over.`,
        );
    ...
    private spawnFood() {
        ...
        if (food.body instanceof Phaser.Physics.Arcade.Body) {
            // Randomize peak height by adjusting velocityY
            // Higher (more negative) velocity = higher toss
            const minPeak = -400; // Lower toss
            const maxPeak = -700; // Higher toss
            food.body.setVelocityY(Phaser.Math.Between(minPeak, maxPeak));
